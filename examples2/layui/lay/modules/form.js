/*
 * 非原始版本，wangming2017-12-15月改版，增加下拉多选
 */
      ...
      select: function(){
        ...
        ,events = function(reElem, disabled, isSearch){
          ...
          var notOption = function(value, callback, origin){
        	  var mul = !!select.attr('lay-mul'); //获取过滤器
        	  if(!mul){
        		  var num = 0;
          ...
          dds.on('click', function(){
            ...
            var mul = !!select.attr('lay-mul'); //获取过滤器
            let build=function(domArr){
            	let textArr=[],valueArr=[];
            	$.each(domArr,function(i,o){
            		textArr.push("【"+$(o).text()+"】");
            		valueArr.push($(o).attr('lay-value'));
            	});
            	return {textArr:textArr,valueArr:valueArr};
            };
            ...
            if(mul){
            	if(othis.hasClass(THIS)){
            		...
            		let domArr=othis.siblings("."+THIS);
            		let setArr=build(domArr);
            		
            		input.val(setArr.textArr.join("，"));
            		select.data("value",setArr.valueArr.join(","))
            	}else {
            		othis.addClass(THIS);
            		let domArr=othis.siblings("."+THIS);
            		domArr.push(othis);
