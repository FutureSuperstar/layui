...
layui.define(function(exports){
  ...
  Class.prototype.view = function(){
    ...
    ,views = {
      ...
      ,more: '<a href="javascript:;" class="layui-laypage-more">查看更多页码</a>'
  ...
  Class.prototype.jump = function(elem, isskip){
    ...
    for(var i = 0, len = childs.length; i < len; i++){
      ...
      console.log(config.more.show,childs[i].className.indexOf('layui-laypage-more'));
      if(config.more.show&&childs[i].className.indexOf('layui-laypage-more')>-1){
    	  laypage.on(childs[i], 'click', function(){
    		  $.ajax({
    			  url:config.more.url,
    			  success:function(data){
    				  if(data.success){
    					  config.count = data.data;
    					  that.render();
    				  }else{
    					  throw '服务端错误：'+data.errorMsg;
    				  }
    			  }
    		  });
    	  });
